...
- (void)createView {
    ...
    _recordingbackImage.image = [UIImage imageNamed:QMChatUIImagePath(@"RecordingBkg")];
    [self addSubview:_recordingbackImage];
    
    ///叹号图片
    _markImage = [[UIImageView alloc] init];
    _markImage.frame = CGRectMake(70, 28, 10, 64);
    _markImage.image = [UIImage imageNamed:QMChatUIImagePath(@"RecordMark")];
    ...
    _volumeImage.image = [UIImage imageNamed:QMChatUIImagePath(@"RecordingSignal001")];
    ...
    _backImage.image = [UIImage imageNamed:QMChatUIImagePath(@"RecordCancel")];
    ...
    _warningLabel.text = QMUILocalizableString(button.cancelsend);
...
- (void)updateImageWithPower:(float)power {
    ...
    _volumeImage.image = [UIImage imageNamed:[NSString stringWithFormat:QMChatUIImagePath(@"RecordingSignal00%d"), no]];
}
...
- (void)changeViewStatus:(QMIndicatorStatus)status {
    switch (status) {
        case QMIndicatorStatusShort:
            ...
            _warningLabel.text = QMUILocalizableString(title.speakS);
            break;
        case QMIndicatorStatusLong:
            ...
            [_countLabel setHidden:YES];
            _warningLabel.backgroundColor = [UIColor clearColor];
            _warningLabel.text = QMUILocalizableString(title.speakL);
            break;
        ...
        case QMIndicatorStatusCancel:
            ...
            _warningLabel.text = QMUILocalizableString(button.recorder_want_cancel);
            break;
        default:
            break;
    }

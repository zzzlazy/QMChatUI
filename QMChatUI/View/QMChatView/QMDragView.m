...
@interface QMDragView ()<UIGestureRecognizerDelegate>
/**
 内容view
 */
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{//拖动中
            ...
            switch (self.dragDirection) {
                case QMDragDirectionAny:
                    dx = point.x - self.startPoint.x;
                    dy = point.y - self.startPoint.y;
                    break;
                case QMDragDirectionHorizontal:
                    dx = point.x - self.startPoint.x;
                    dy = 0;
                    break;
                case QMDragDirectionVertical:
                    dx = 0;
                    dy = point.y - self.startPoint.y;
                    break;
                default:
...
- (UIButton *)button{
    if (_button==nil) {
        ...
        [_button layoutButtonWithEdgeInsetsStyle:QMButtonEdgeInsetsStyleLeft imageTitleSpace:5];
        [_button setTitleColor:[UIColor colorWithHexString:@"#459AE9"] forState:UIControlStateNormal];
